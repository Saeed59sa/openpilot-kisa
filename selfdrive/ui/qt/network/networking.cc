...
public:
  explicit RemoteAccessWidget(QWidget *parent = nullptr) : QWidget(parent) {
    ...
    QLabel *title = new QLabel(tr("Login Code"));
    title->setStyleSheet("color: white;");
    lay->addWidget(title);

    code_label_ = new QLabel("");
    code_label_->setStyleSheet("color: white;");
    code_label_->setWordWrap(true);
    lay->addWidget(code_label_);
...
private:
  void refreshURL() {
    ...
    QString code = current_url_.section('/', -1);
